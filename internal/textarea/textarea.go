...
import (
	...
	"github.com/wheelibin/dbee/internal/runeutil"
)
...
// Highlight is a slide of symbol strings and a style to apply to those symbols
...
type Model struct {
	...
	// Highlights is a slice of highlight definitions
	...
	for l, line := range m.value {
		...
		for wl, wrappedLine := range wrappedLines {
			...
			if m.row == l && lineInfo.RowOffset == wl {
				...
				s.WriteString(m.highlightText(txt))
...
func (m Model) highlightText(txt string) string {
	replacerInput := []string{}
	for _, h := range m.Highlights {
		for _, s := range h.Symbols {
			replacerInput = append(replacerInput, s, h.Style.Render(s))
		}
	}
	if len(replacerInput) == 0 {
		return txt
	}
